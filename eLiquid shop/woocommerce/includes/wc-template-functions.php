...
if ( ! function_exists( 'woocommerce_get_product_thumbnail' ) ) {
	...
	function woocommerce_get_product_thumbnail( $size = 'shop_catalog', $deprecated1 = 0, $deprecated2 = 0 ) {
		...
		if ( has_post_thumbnail() ) {
			return get_the_post_thumbnail( $post->ID, "", array('class' => 'img-responsive') );
...
if ( ! function_exists( 'woocommerce_form_field' ) ) {
	...
	function woocommerce_form_field( $key, $args, $value = null ) {
		...
		switch ( $args['type'] ) {
			case 'country' :
				...
				if ( sizeof( $countries ) == 1 ) {
					...
					$field .= '<input type="hidden" name="' . esc_attr( $key ) . '" id="' . esc_attr( $args['id'] ) . '" value="' . current( array_keys($countries ) ) . '" ' . implode( ' ', $custom_attributes ) . ' class="country_to_state form-control input-text" />';

				} else {

					$field = '<select name="' . esc_attr( $key ) . '" id="' . esc_attr( $args['id'] ) . '" class="country_to_state country_select input-text' . esc_attr( implode( ' ', $args['input_class'] ) ) .'" ' . implode( ' ', $custom_attributes ) . '>'
			...
			case 'state' :
				...
				if ( is_array( $states ) && empty( $states ) ) {
					...
					$field .= '<select name="' . esc_attr( $key ) . '" id="' . esc_attr( $args['id'] ) . '" class="state_select form-control input-text' . esc_attr( implode( ' ', $args['input_class'] ) ) .'" ' . implode( ' ', $custom_attributes ) . ' placeholder="' . esc_attr( $args['placeholder'] ) . '">
		...
		if ( ! empty( $field ) ) {
			...
			if ( $args['description'] ) {
				$field_html .= '<span class="description form-control">' . esc_html( $args['description'] ) . '</span>';
			}
